...
class SerialTriggerHandler(private val context: Context) {
    ...
    suspend fun initializeConnection() = withContext(Dispatchers.IO) {
    ...
    suspend fun sendTrigger(triggerValue: String) = withContext(Dispatchers.IO) {
        usbPort?.write(triggerValue.toByteArray(), 1000)
    }
